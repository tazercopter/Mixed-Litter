...
public class DolphinVariant extends MobVariant {
    public static final MapCodec<DolphinVariant> CODEC = RecordCodecBuilder.mapCodec(
            ...
            ).apply(instance, DolphinVariant::new)
    );
    ...
    public DolphinVariant(int weight, HolderSet<Biome> biomes, ResourceLocation texture) {
        super(List.of(EntityType.DOLPHIN));
        this.weight = weight;
        this.biomes = biomes;
    ...
    public MobVariant select(LivingEntity entity, LevelAccessor level, List<? extends MobVariant> animalVariants) {
        ...
        List<DolphinVariant> variants = new ArrayList<>();
        animalVariants.forEach(animalVariant -> {
            if (animalVariant instanceof DolphinVariant variant) variants.add(variant);
        });

        List<DolphinVariant> defaultVariants = variants.stream().filter(v -> v.biomes.size() == 0).toList();
        List<DolphinVariant> biomeVariants = variants.stream().filter(v -> v.biomes.size() > 0 && v.biomes.contains(level.getBiome(entity.blockPosition()))).toList();
        ...
        if (!biomeVariants.isEmpty()) {
            ...
            for (DolphinVariant variant : biomeVariants) {
            ...
            int randomWeight = level.getRandom().nextInt(totalWeight);
            for (DolphinVariant variant : defaultVariants) {
                cumulativeWeight += variant.weight;
                if (randomWeight < cumulativeWeight) {
                    selectedVariant = variant;
                    break;
