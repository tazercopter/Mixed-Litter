...
public class FoxVariant extends MobVariant {
    public static final MapCodec<FoxVariant> CODEC = RecordCodecBuilder.mapCodec(
            instance -> instance.group(
                    ...
                    ResourceLocation.CODEC.fieldOf("sleeping_texture").forGetter(v -> v.sleepingTexture)
            ).apply(instance, FoxVariant::new)
    );
    ...
    public FoxVariant(int weight, HolderSet<Biome> biomes, ResourceLocation texture, ResourceLocation sleepingTexture) {
        super(List.of(EntityType.FOX));
        this.weight = weight;
        this.biomes = biomes;
    ...
    public MobVariant select(LivingEntity entity, LevelAccessor level, List<? extends MobVariant> animalVariants) {
        ...
        List<FoxVariant> variants = new ArrayList<>();
        animalVariants.forEach(animalVariant -> {
            if (animalVariant instanceof FoxVariant variant) variants.add(variant);
        });

        List<FoxVariant> defaultVariants = variants.stream().filter(v -> v.biomes.size() == 0).toList();
        List<FoxVariant> biomeVariants = variants.stream().filter(v -> v.biomes.size() > 0 && v.biomes.contains(level.getBiome(entity.blockPosition()))).toList();
        ...
        if (!biomeVariants.isEmpty()) {
            ...
            for (FoxVariant variant : biomeVariants) {
            ...
            int randomWeight = level.getRandom().nextInt(totalWeight);
            for (FoxVariant variant : defaultVariants) {
                cumulativeWeight += variant.weight;
                if (randomWeight < cumulativeWeight) {
                    selectedVariant = variant;
                    break;
