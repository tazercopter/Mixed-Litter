...
public class ZombieVariant extends MobVariant {
    public static final MapCodec<ZombieVariant> CODEC = RecordCodecBuilder.mapCodec(
    ...
    public ZombieVariant(int weight, HolderSet<Biome> biomes, ResourceLocation texture) {
        super(List.of(EntityType.ZOMBIE));
        this.weight = weight;
        this.biomes = biomes;
    ...
    public MobVariant select(LivingEntity entity, LevelAccessor level, List<? extends MobVariant> animalVariants) {
        ...
        List<ZombieVariant> variants = new ArrayList<>();
        animalVariants.forEach(animalVariant -> {
            if (animalVariant instanceof ZombieVariant variant) variants.add(variant);
        });

        List<ZombieVariant> defaultVariants = variants.stream().filter(v -> v.biomes.size() == 0).toList();
        List<ZombieVariant> biomeVariants = variants.stream().filter(v -> v.biomes.size() > 0 && v.biomes.contains(level.getBiome(entity.blockPosition()))).toList();
        ...
        if (!biomeVariants.isEmpty()) {
            ...
            for (ZombieVariant variant : biomeVariants) {
            ...
            int randomWeight = level.getRandom().nextInt(totalWeight);
            for (ZombieVariant variant : defaultVariants) {
                cumulativeWeight += variant.weight;
                if (randomWeight < cumulativeWeight) {
                    selectedVariant = variant;
                    break;
