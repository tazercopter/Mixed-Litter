...
public class HuskVariant extends MobVariant {
    public static final MapCodec<HuskVariant> CODEC = RecordCodecBuilder.mapCodec(
            ...
            ).apply(instance, HuskVariant::new)
    );
    ...
    public HuskVariant(int weight, HolderSet<Biome> biomes, ResourceLocation texture) {
        super(List.of(EntityType.HUSK));
        this.weight = weight;
        this.biomes = biomes;
    ...
    public MobVariant select(LivingEntity entity, LevelAccessor level, List<? extends MobVariant> animalVariants) {
        ...
        List<HuskVariant> variants = new ArrayList<>();
        animalVariants.forEach(animalVariant -> {
            if (animalVariant instanceof HuskVariant variant) variants.add(variant);
        });

        List<HuskVariant> defaultVariants = variants.stream().filter(v -> v.biomes.size() == 0).toList();
        List<HuskVariant> biomeVariants = variants.stream().filter(v -> v.biomes.size() > 0 && v.biomes.contains(level.getBiome(entity.blockPosition()))).toList();
        ...
        if (!biomeVariants.isEmpty()) {
            ...
            for (HuskVariant variant : biomeVariants) {
            ...
            int randomWeight = level.getRandom().nextInt(totalWeight);
            for (HuskVariant variant : defaultVariants) {
                cumulativeWeight += variant.weight;
                if (randomWeight < cumulativeWeight) {
                    selectedVariant = variant;
                    break;
