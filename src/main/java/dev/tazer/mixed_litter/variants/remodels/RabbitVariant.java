...
public class RabbitVariant extends MobVariant {
    public static final MapCodec<RabbitVariant> CODEC = RecordCodecBuilder.mapCodec(
    ...
    public RabbitVariant(int weight, HolderSet<Biome> biomes, ResourceLocation texture, ResourceLocation babyTexture) {
    ...
    public MobVariant select(LivingEntity entity, LevelAccessor level, List<? extends MobVariant> animalVariants) {
        ...
        List<RabbitVariant> variants = new ArrayList<>();
        animalVariants.forEach(animalVariant -> {
            if (animalVariant instanceof RabbitVariant variant) variants.add(variant);
        });

        List<RabbitVariant> defaultVariants = variants.stream().filter(v -> v.biomes.size() == 0).toList();
        List<RabbitVariant> biomeVariants = variants.stream().filter(v -> v.biomes.size() > 0 && v.biomes.contains(level.getBiome(entity.blockPosition()))).toList();
        ...
        if (!biomeVariants.isEmpty()) {
            ...
            for (RabbitVariant variant : biomeVariants) {
            ...
            int randomWeight = level.getRandom().nextInt(totalWeight);
            for (RabbitVariant variant : defaultVariants) {
                cumulativeWeight += variant.weight;
                if (randomWeight < cumulativeWeight) {
                    selectedVariant = variant;
                    break;
